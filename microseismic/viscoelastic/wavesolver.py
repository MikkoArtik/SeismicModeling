...
from examples.seismic.viscoelastic.operators import create_operator
from examples.seismic.viscoelastic.operators import create_operator_domain
...
class ViscoelasticWaveSolver(object):
    ...
    def op_fwd(self, save=None):
        ...
        return create_operator(self.model, save=save, geometry=self.geometry,
...
class ViscoelasticWaveSolverDomains(object):
    """
    Solver object that provides operators for seismic inversion problems
    and encapsulates the time and space discretization for a given problem
    setup.
    ...
    This is an experimental staggered grid viscoelastic modeling kernel.
    """
    def __init__(self, model, geometry, dt, is_using_right_z_reflection,
                 space_order=4, **kwargs):
        self.model = model
        self.geometry = geometry

        self.space_order = space_order
        self.dt = dt
        self.is_using_right_z_reflection = is_using_right_z_reflection
        # Cache compiler options
        self._kwargs = kwargs
    ...
    def op_fwd(self, save=None):
        """Cached operator for forward runs with buffered wavefield"""
        return create_operator_domain(
            self.model, save=save, geometry=self.geometry,
            space_order=self.space_order,
            is_using_right_z_reflection=self.is_using_right_z_reflection,
            **self._kwargs)
    ...
    def forward(self, save=None, **kwargs):
        ...
        # Create a new receiver object to store the result
        rec_vx = Receiver(name='rec_vx', grid=self.model.grid,
                          time_range=self.geometry.time_axis,
                          coordinates=self.geometry.rec_positions)
        rec_vy = Receiver(name='rec_vy', grid=self.model.grid,
                          time_range=self.geometry.time_axis,
                          coordinates=self.geometry.rec_positions)
        rec_vz = Receiver(name='rec_vz', grid=self.model.grid,
                          time_range=self.geometry.time_axis,
                          coordinates=self.geometry.rec_positions)
        ...
        # Execute operator and return wavefield and receiver data
        self.op_fwd(save).apply(src=src, mu=mu, qp=qp, lam=lam, qs=qs,
                                irho=irho, dt=self.dt,
                                rec_vx=rec_vx, rec_vy=rec_vy, rec_vz=rec_vz,
                                **kwargs)
        return v, rec_vx, rec_vy, rec_vz
