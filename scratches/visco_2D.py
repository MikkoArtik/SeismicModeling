...
from seismic import ModelViscoelastic, plot_image
...
qp_model = np.zeros(shape=shape, dtype=np.float)
qs_model = np.zeros(shape=shape, dtype=np.float)
...
for layer in layers[:-1]:
    ...
    qp_model[:, top_index: bottom_index] = qp_val
    vs_model[:, top_index: bottom_index] = vs_val
    qs_model[:, top_index: bottom_index] = qs_val
...
qp_model[:, top_index:] = qp_val
vs_model[:, top_index:] = vs_val
qs_model[:, top_index:] = qs_val
...
model = ModelViscoelastic(shape=shape, spacing=delta_size,
                          space_order=space_order, vp=vp_model,
                          vs=vs_model, b=1/density_model, qp=qp_model,
                          qs=qs_model, nbl=nbl, origin=(0, 0),
...
lam, mu, irho, qp, qs, damp = model.lam, model.mu, model.b, model.qp, \
                              model.qs, model.damp

t_s = (sp.sqrt(1+1/qp**2)-1/qp) / source_frequency_kHz
t_ep = 1/(source_frequency_kHz ** 2 * t_s)
t_es = (1 + source_frequency_kHz * qs * t_s) / (source_frequency_kHz * qs -
                                                source_frequency_kHz ** 2 *
                                                t_s)
...
tau = TensorTimeFunction(name='t', grid=model.grid, space_order=space_order,
                         time_order=time_order)
r = TensorTimeFunction(name='r', grid=model.grid, space_order=space_order,
                       time_order=time_order)
...
# Stress equations:
u_t = Eq(tau.forward, damp * (r.forward + tau + s * (lam * t_ep / t_s *
         diag(div(v.forward)) + mu * t_es / t_s * symm_grad)),
         subdomain=model.grid.subdomains['right_y'])

# Memory variable equations:
u_r = Eq(r.forward, damp * (r - s / t_s * (r + mu * (t_es / t_s - 1) *
         symm_grad + lam * (t_ep / t_s - 1) * diag(div(v.forward)))))
...
op = Operator([u_v, u_r, u_t] + src_rec_expr, subs=model.grid.spacing_map)
op.apply(dt=dt)
